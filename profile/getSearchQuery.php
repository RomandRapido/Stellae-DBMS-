...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $getPapersQuery = "SELECT posts.*,
            ...
            if ($_POST['type']=='public'){
                $getPapersQuery .= "WHERE posts.privacy = 'public' AND posts.author_id = $userId ";
            }else if ($_POST['type'] == 'private'){
                $getPapersQuery .= "WHERE posts.privacy = 'private' AND posts.author_id = $userId ";
            }else if ($_POST['type']=='liked'){
                ...
                GROUP_CONCAT(interests.interest_name) AS interest_names,
                AVG((post_ratings.scale_id_thought_quality + post_ratings.scale_id_connection_response + post_ratings.scale_id_idea_organization + post_ratings.scale_id_language_accuracy + post_ratings.scale_id_references_citations)/5) AS avg_rating,
                IF(post_likes.user_liker IS NOT NULL, 1, 0) AS is_liked_by_current_user
                FROM 
                    posts
                INNER JOIN 
                    users ON posts.author_id = users.user_id
                LEFT JOIN 
                    post_interests ON posts.post_id = post_interests.post_id
                LEFT JOIN 
                    interests ON post_interests.interest_id = interests.interest_id
                LEFT JOIN 
                    post_ratings ON posts.post_id = post_ratings.post_id
                LEFT JOIN 
                    post_likes ON posts.post_id = post_likes.post_id
                WHERE post_likes.user_liker = $userId ";
            }
